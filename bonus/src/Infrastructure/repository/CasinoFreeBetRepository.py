...
class DjangoCasinoFreeBetRepository(CasinoFreeBetRepository):
    ...
    def save(self, freebet: CasinoFreeBet) -> CasinoFreeBet:
        ...
        with transaction.atomic():
            if freebet.id:
                # Update existing
                model = CasinoFreeBetModel.objects.select_for_update().get(
                    id=freebet.id
                )
    ...
    def find_active_by_tenant(
        ...
        models = (
            CasinoFreeBetModel.objects.filter(
                tenant_id=tenant_id, status=CasinoFreeBetModel.Status.ACTIVE
            )
            .annotate(
    ...
    def find_expiring_soon(self, minutes_threshold: int) -> list[CasinoFreeBet]:
        ...
        # expiry_time = created_at + expiry_minutes (as interval)
        models = (
            CasinoFreeBetModel.objects.filter(status=CasinoFreeBetModel.Status.ACTIVE)
            .annotate(
                expiry_time=ExpressionWrapper(
                    F("created_at") + timedelta(minutes=1) * F("expiry_minutes"),
                    output_field=DateTimeField(),
    ...
    def _update_model_from_entity(
        self, model: "CasinoFreeBetModel", entity: CasinoFreeBet
    ) -> None:
