...
from bonus.src.Domain.value_objects import FreebetCurrency, CasinoFreeBetStatus
from bonus.src.Domain.exceptions import FreebetError, FreebetExpiredError, InvalidFreebetStateError
    ...
    def post(self, request):
        ...
        try:
            ...
            request_dto = CasinoCreateFreeBetRequestDto(
                ...
                initial_status=CasinoFreeBetStatus(data.get('initial_status', 'INACTIVE')),
            )
            
            use_case = CreateCasinoFreeBetUseCase(get_repository())
            result = use_case.execute(request_dto)
            
            logger.info(f"Created freebet: {result.public_id} for tenant: {data['tenant_id']}")
            return JsonResponse({"success": True, "data": self._serialize_response(result)}, status=201)
...
class CasinoFreeBetDetailView(View):
    ...
    def patch(self, request, public_id):
        ...
        try:
            ...
            logger.warning(f"Missing required field in status update for {public_id}: {e}")
            return JsonResponse({
                "success": False,
