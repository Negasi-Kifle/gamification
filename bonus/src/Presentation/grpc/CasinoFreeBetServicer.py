...
from ...Domain.exceptions import FreebetError, FreebetExpiredError, InvalidFreebetStateError
...
class CasinoFreeBetServicer(bonus_pb2_grpc.CasinoFreeBetServiceServicer):
    ...
    def CreateFreebet(self, request, context):
        ...
        try:
            ...
            initial_status = STATUS_MAP.get(request.initial_status, CasinoFreeBetStatus.INACTIVE)
    ...
    def UpdateStatus(self, request, context):
        ...
        try:
            ...
            logger.info(f"gRPC UpdateStatus success: {request.public_id} -> {new_status.value}")
            return self._to_proto_response(result)
            ...
            logger.warning(f"gRPC UpdateStatus invalid state: {request.public_id} - {e}")
            context.set_code(400)  # FAILED_PRECONDITION
            context.set_details(str(e))
            return self._empty_response()
        except ValueError as e:
    ...
    def GetExpiring(self, request, context):
        ...
        try:
            ...
            logger.debug(f"gRPC GetExpiring called with threshold: {hours_threshold} hours")
    ...
    def _to_proto_response(self, dto: CasinoFreeBetResponseDto) -> bonus_pb2.FreebetResponse:
        """Convert response DTO to proto Message."""
        data = {
            "public_id": str(dto.public_id),
            "name": dto.name,
            "currency": str(dto.currency), # Ensure it's a string if proto expects string
            "game_id": dto.game_id,
